...
Nous allons profiter de cet atelier pour l'installer et voir une manière d'installer des ressources dans un cluster.
...
- création d'un role et cluster role ainsi que les bindings associés
- création du déploiement qui va deployer les containers du dashboard
...
> Si vous utilisez chrome pour l'atelier, activez les certificats auto-signés sur localhost
> tapez _chrome://flags/#allow-insecure-localhost_ dans l'url et activer l'option

- Se placer dans le namespace du dashboard et consulter les pods déployés, noter les ports utilisés avec _k describe po_
- Consulter le service du dashboard (kubernetes-dashboard) les cibles et les ports utilisés (noter comment le service sélectionne les pods qui vont recevoir le flux)
- Utiliser la commande _k port-forward_ pour bind un port local de votre machine au port du service dashboard
- Comparer le retour de la commande et la ligne de commande saisie en s'appuyant sur le _describe_ fait précédement
...
Nous allons dans cet exercice créer un user avec une autre méthode de modification de ressources kubernetes.

- Créer un user technique (serviceaccount) 'dashboard-admin' dans le namespace kubernetes-dashboard en utilisant _k create sa_
- Lister les roles existants au niveau cluster (clusterrole). Kubernetes propose déjà des rôles, consulter notamment le cluster rôle 'admin' au format _yaml_
- Créer un clusterrolebinding en précisant uniquement le rôle 'admin' _k create clusterrolebinding_
- Consulter au format _yaml_ le clusterrolebinding créé précédement
